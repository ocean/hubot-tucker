...
const allQuotes = secondPerson.concat(thirdPerson, restOfQuotes);

module.exports = function(robot) {
  if (!process.env.HUBOT_TUCKER_OFF) {
    robot.respond(/tucker (.+)/i, function(res) {
      let quote;
      const person = res.match[1];
      if (person === "me") {
        quote = res.random(thirdPerson);
        return res.send(`Malcolm was talking about you earlier, he said \"${quote}\"`);
      } else if (person.search(/malc|malcolm|tucker|himself/i) > -1) {
        return res.send("I'm watching you, you cheeky fucker.");
      } else {
        quote = res.random(secondPerson);
        return res.send(`I heard Malcolm talking to ${person}, he said \"${quote}\"`);
      }
    });

    robot.hear(/tucker/i, function(res) {
      const message = res.message.text;
      const re = new RegExp(robot.name, 'i');
      if (message.search(re) === -1) {
        const quote = res.random(allQuotes);
        if (process.env.HUBOT_TUCKER_BYLINE) {
          return res.send(`\"${quote}\" -- Malcolm Tucker`);
        } else {
    ...
    if (!process.env.HUBOT_LESS_MALCOLM) {
      return robot.hear(/malc(\s*|olm)/i, function(res) {
        const message = res.message.text;
        if ((message.search(/tucker/i) === -1) && (message.search(/Malcolm was talking/i) === -1) && (message.search(/heard Malcolm talking/i) === -1)) {
          const quote = res.random(allQuotes);
          if (process.env.HUBOT_TUCKER_BYLINE) {
            return res.send(`\"${quote}\" -- Malcolm Tucker`);
          } else {
